...
import (
	...
	"github.com/wind-river/cloud-platform-deployment-manager/internal/controller/host"
	"github.com/wind-river/cloud-platform-deployment-manager/internal/controller/system"
	webhookv1 "github.com/wind-river/cloud-platform-deployment-manager/internal/webhook/v1"
...
func main() {
	...
	if err = (&controller.DataNetworkReconciler{
	...
	if err = (&controller.HostProfileReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err = (&controller.PlatformNetworkReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err = (&controller.PtpInstanceReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err = (&controller.PtpInterfaceReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err = webhookv1.SetupDataNetworkWebhookWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create webhook", "webhook", "DataNetwork")
		os.Exit(1)
	}
	if err = webhookv1.SetupHostProfileWebhookWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create webhook", "webhook", "HostProfile")
		os.Exit(1)
	}
	if err = webhookv1.SetupHostWebhookWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create webhook", "webhook", "Host")
		os.Exit(1)
	}
	if err = webhookv1.SetupPlatformNetworkWebhookWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create webhook", "webhook", "PlatformNetwork")
		os.Exit(1)
	}
	if err = webhookv1.SetupAddressPoolWebhookWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create webhook", "webhook", "AddressPool")
		os.Exit(1)
	}
	if err = webhookv1.SetupPtpInstanceWebhookWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create webhook", "webhook", "PtpInstance")
		os.Exit(1)
	}
	if err = webhookv1.SetupPtpInterfaceWebhookWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create webhook", "webhook", "PtpInterface")
		os.Exit(1)
	}
	if err = webhookv1.SetupSystemWebhookWithManager(mgr); err != nil {
